...
def test_LayBlockDRT_init():
    lay_drt = LayBlockDRT(10, in_lay_norm=False)
    print(lay_drt)


def test_LayBlockDRT_init_more():
    lay_drt = LayBlockDRT(
        in_width=           10,
        do_scaled_dns=      True,
...
def test_LayBlockDRT_base():
    ...
    out = lay_drt(inp)
    print(out)
    assert out['out'].shape[-1] == in_width
    assert out['zeroes'].shape[-1] == in_width


def test_LayBlockDRT_kwargs():
    in_width = 10
    inp = torch.rand(in_width) - 0.5
    ...
    print(out)
    assert out['out'].shape[-1] == in_width
    assert out['zeroes'].shape[-1] == in_width * 4


def test_LayBlockDRT_device():
    in_width = 10
    inp = torch.rand(in_width) - 0.5
    ...
    print(out)
    assert str(out['out'].device) == 'cpu'


def test_LayBlockDRT_double():
    in_width = 10
    inp = torch.rand(in_width) - 0.5
...
def test_EncDRT():
    ...
    print(enc_drt)
    assert len(list(enc_drt.children())) == 2
    out = enc_drt(inp)
    print(out)
    assert out['zeroes'].shape[0] == 6 * in_width * 4

    enc_drt = EncDRT(in_width, n_layers=5, lay_width=2 * in_width, dns_scale=3)
    print(enc_drt)
    assert len(list(enc_drt.children())) == 7
    out = enc_drt(inp)
    print(out)
    assert out['zeroes'].shape[0] == 5 * 2 * in_width * 3
    ...
    out = enc_drt(inp)
    print(out)
    assert out['zeroes'].shape[0] == 4 * 16 * 3


def test_LayBlockCNN_base_init_call():
    n_filters = 6
    ...
    assert list(out.shape) == [4, 6] and state is None and zeroes is None

    lay_cnn = LayBlockCNN(n_filters)
    cnn_out = lay_cnn(inp)
    zeroes = cnn_out['zeroes']
    assert list(zeroes.shape) == [6]


def test_LayBlockCNN_no_pad():
    n_filters = 6
...
def test_LayBlockCNN_input_shapes():
    n_filters = 6
    lay_cnn = LayBlockCNN(n_filters)
    print(lay_cnn)
...
def test_LayBlockCNN_history():
    n_filters = 6
    lay_cnn = LayBlockCNN(n_filters)

    history = lay_cnn.get_zero_history()
    print(f'history.shape: {history.shape}')
    assert list(history.shape) == [2, 6]
...
def test_LayBlockCNN_moving_history():
    n_filters = 6
    lay_cnn = LayBlockCNN(n_filters)
    ...
    for ix in range(5):
        ...
        print(state_prev[1])
        print(state[0])
        assert torch.equal(state_prev[1], state[0])


def test_LayBlockCNN_base_encoder():
    n_time = 6
    n_filters = 4
    ...
    for kernel_size in [3, 5, 7, 9]:
        ...
        assert inp.shape == out['out'].shape
        assert out['state'] is None


def test_LayBlockCNN_base_casual():
    n_time = 6
    n_filters = 4
    inp = torch.rand(n_time, n_filters)
    print(f'inp shape {inp.shape}')
    ...
    for kernel_size in [3, 5, 7, 9]:
        ...
        assert list(zero_history.shape) == [kernel_size - 1, n_filters]
        ...
        assert out['state'].shape == zero_history.shape


def test_LayBlockCNN_kwargs():
    n_filters = 4
...
def test_LayBlockCNN_more():
    ...
    assert o_shape == list(out['state'].shape)


def test_EncCNN_base_init_call():
    in_features = 8
    ...
    print(f'out.shape: {out.shape}')
    assert list(out.shape) == [7, 5, in_features] and state is None and list(zeroes.shape) == [48]


def test_EncCNN_input_shapes():
    in_features = 8
    enc = EncCNN(in_features)
...
def test_EncCNN_init_raises():
    with pytest.raises(TorchnessException):
        EncCNN(in_features=6, kernel_size=4)


def test_EncCNN_no_pad():
    in_features = 8
    ...
    out = enc_out['out']
    print(out.shape)
    assert list(out.shape) == [5, 8, 8]


def test_EncCNN_history():
    in_features = 8
    enc = EncCNN(in_features)

    history = enc.get_zero_history()
    print(f'history.shape: {history.shape}')
    assert list(history.shape) == [6, 2, 8]
...
def test_EncCNN_casual():
    ...
    state = enc_out['state']
    print(f'state.shape: {state.shape}')
    assert list(state.shape) == [7, 6, 2, 8]


def test_EncCNN_kwargs():
    n_filters = 48
    in_features = 32
    ...
    in_sh[-1] = n_filters
    assert list(enc_out['out'].shape) == in_sh

    zero_history = enc.get_zero_history()
    print(zero_history.shape)
    assert list(zero_history.shape) == [5, 6, 48]


def test_EncCNN_shared():
    with pytest.raises(TorchnessException):
        EncCNN(in_features=6, kernel_size=4)
    ...
    print(enc_out['out'].shape)
    assert list(enc_out['out'].shape) == list(inp.shape)

    zero_history = enc.get_zero_history()
    print(zero_history.shape)
    assert list(zero_history.shape) == [6, 2, 128]


def test_LayBlockTNS_base():
    in_features = 64
...
def test_EncTNS_base():
    in_features = 64
    enc = EncTNS(num_layers=4, d_model=in_features)
    print(enc)
    assert enc.pos_emb is None
    ...
    assert list(out.shape) == [16, 32, in_features] and list(zeroes.shape) == [1024]


def test_EncTNS_input_shapes():
    in_features = 16
    enc = EncTNS(num_layers=4, d_model=in_features)
...
def test_EncTNS_PE():
    ...
    print(enc)
    print(enc.pos_emb)
    assert enc.pos_emb is not None
    out = enc(inp)
    print(out['out'].shape)
...
def test_EncTNS_TAT():
    ...
    enc = EncTNS(num_layers=4, num_layers_TAT=2, d_model=in_features)
    print(enc)
    out = enc(inp)
    print(out['out'].shape)


def test_EncTNS_TAT_shared():
    in_features = 64
    seq_len = 32
    inp = torch.rand(16, seq_len, in_features)
    print(inp.shape)
